...
export default function Ticket({ bestValue = false, soldOut = false, ticket}: { bestValue?: boolean, soldOut?: boolean, ticket: ITicket }) {
	return (
		<div tabIndex={0} className={`group relative flex flex-col md:flex-row gap-6 p-6 rounded-xl border border-surface-border focus:border-primary/20! bg-linear-to-r from-surface-dark to-[#3E3538] hover:border-primary transition-all duration-300 shadow-lg shadow-black/20 ${soldOut && "border-surface-border! bg-surface-dark opacity-60 cursor-not-allowed grayscale"}`}>
			...
			<div className="flex flex-col items-end justify-between gap-4 md:min-w-35">
				...
				<div className="flex items-center bg-background-dark rounded-full border border-surface-border p-1">
					<button className={`size-8 rounded-full flex items-center justify-center text-gray-400 hover:text-white hover:bg-surface-border transition-colors ${soldOut && "cursor-not-allowed"}`} disabled={soldOut}>
						<span className="material-symbols-outlined text-lg">remove</span>
					</button>
					<span className="w-8 text-center text-white font-medium">2</span>
					<button className={`size-8 rounded-full flex items-center justify-center text-white bg-primary hover:bg-primary-dark transition-colors shadow-lg shadow-primary/20 ${soldOut && "cursor-not-allowed"}`} disabled={soldOut}>
