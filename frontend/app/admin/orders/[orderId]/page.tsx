...
export default function Page({params}: {params: Promise<PageProps>}) {
	...
	useEffect(() => {
		(async () => {
			const orderResponse = await api.get(`/orders/${routeParams.orderId}`)
	...
	const deleteOrder = async () => {
		const deleteResponse = await api.delete(`/orders/${routeParams.orderId}`);
		router.replace(paths.orders);
	}
	
	const confirmOrder = async () => {
		const confirmResponse = await api.post(`/orders/${routeParams.orderId}/confirm`);
		router.refresh();
	}
			...
			<div className="container mx-auto max-w-300 p-4 lg:p-8 flex flex-col gap-6">
				...
				<div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 border-b border-white/5 pb-6">
					<div className="flex flex-col gap-2">
						...
						<p className="text-gray-400 text-base lg:text-lg">Manage details for order placed by <span className="text-white font-medium">{order.customerName}</span></p>
					</div>
					<div className="flex gap-3">
						<button onClick={deleteOrder} className="px-4 py-2 rounded-lg border border-red-500/30 text-red-400 hover:bg-red-500/10 hover:border-red-500/50 transition-all flex items-center gap-2 text-sm font-medium">
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
					...
					<div className="flex flex-col gap-6">
						<StatusManager order={order} confirmOrder={confirmOrder} />
					</div>
				</div>
			</div>
		</main>
